...
WeedFSClient.prototype = {
            ...
            self._assign(opts).then(function (finfo) {
                ...
                for (var i = 0; i < opts.count; i++) {
                    proms.push(new Promise(function (resolve, reject) {
                        ...
                        var url = opts.count == 1 ? "http://" + finfo.url + "/" + finfo.fid : "http://" + finfo.url + "/" + finfo.fid + "_" + i;
        ...
        return new Promise(function (resolve, reject) {
            self.find(fid).then(function (res) {
                if (res.locations.length) {
                    var req = http.request(url.parse("http://" + res.locations[0].publicUrl + "/" + fid), function (res) {
    ...
    remove: function (fid) {
        ...
        return new Promise(function (resolve, reject) {
            //for now server property is not supported
            if (server) {
            ...
            self.find(fid).then(function (result) {
                ...
                for (var i = 0, len = result.locations.length; i < len; i++) {
                    proms.push(new Promise(function (resolve, reject) {
                        var req = http.request(url.parse("http://" + result.locations[i].publicUrl + "/" + fid), function (res) {
                            if (res.statusCode === 404) {
                                var err = new SeaweedFSError("file '" + fid + "' not found");
                                return reject(err);
                            }
